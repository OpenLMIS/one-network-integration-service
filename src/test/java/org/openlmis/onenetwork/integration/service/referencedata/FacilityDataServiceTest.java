...
public class FacilityDataServiceTest {
  ...
  public void shouldReturnEmptyListWhenNoResponseIsAvailable() {
    ...
    when(restTemplate.exchange(
            ...
            eq(FacilityWrapper.class)))
            .thenReturn(ResponseEntity.noContent().build());

    List<Facility> list = facilityDataService.getAllFacilities();
  ...
  @Test
  public void shouldReturnListOfFacilitiesWhenResponseIsAvailable() {
    Facility facility = new Facility.Builder().build();
    FacilityWrapper facilityWrapper = new FacilityWrapper();
    facilityWrapper.setContent(Collections.singletonList(facility));
    when(restTemplate.exchange(
            any(URI.class),
            eq(HttpMethod.GET),
            any(HttpEntity.class),
            eq(FacilityWrapper.class)))
            .thenReturn(ResponseEntity.of(Optional.of(facilityWrapper)));

    List<Facility> list = facilityDataService.getAllFacilities();

    assertThat(list).isNotEmpty();
    assertThat(list.contains(facility)).isTrue();
  }
}
